// Query Compiler
// -------
const {containsUndefined} = require('../helpers')
...
class QueryCompiler {
  ...
  _ignore = (columns) => ` on conflict (${this.formatter.columnize(columns)}) do nothing`;
  ...
  truncate() {
    ...
    return {
      ...
      output() {
        return this.query({sql: `delete from sqlite_sequence where name = '${table}'`}).catch(noop);
      },
    };
  }

  tableInfo() {
    return {sql: `SELECT * FROM sqlite_master WHERE tbl_name = "${this.single.table}"`}
  }
    ...
    return {
      ...
      output(resp) {
        ...
        for (let val of resp) {
          ...
          if (val.name === column) return columns[val.name]
        }
        return columns;
      },
    };
  ...
  _merge(updates, columns, insert) {
    ...
    if (updates) {
      ...
      let res = []
      for (let column of insertData.columns) {
        const c = this.formatter.wrapString(column.split('.').pop())
        res.push(`${c} = excluded.${c}`)
      }
      sql += res.join(', ')
  ...
  havingBetween(statement) {
    return (
      ...
      ' ' +
      statement.value.map(bind(this.formatter.parameter, this.formatter)).join(' and ')
    );
  }
