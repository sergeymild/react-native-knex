...
class Transaction extends EventEmitter {
  constructor(client, container, config = DEFAULT_CONFIG(), outerTx = null) {
    ...
    if (this._debug) {
      this.logger.debug(`${this.txid}: Starting ${outerTx ? 'nested' : 'top level'} transaction`)
    }
  ...
  query(conn, sql, status, value) {
    const q = this.trxClient
      ...
      .then((res) => {
        ...
        if (status === 2) {
          if (value === undefined) {
            if (this.doNotRejectOnRollback && /^ROLLBACK\b/i.test(sql)) return this._resolver();
    ...
    if (status === 1 || status === 2) this._completed = true;
    return q;
  }
  ...
  async _evaluateContainer(config, container) {
    return this.acquireConnection(config, (connection) => {
      ...
      init.then(() => makeTransactor(this, connection, trxClient))
        .then((transactor) => {
          ...
          if (result && typeof result.then === 'function') {
            result
              .then(transactor.commit)
              .catch(transactor.rollback);
          }
          return null;
...
function makeTransactor(trx, connection, trxClient) {
  const transactor = new Knex(trxClient);
...
function completedError(trx, obj) {
  ...
  console.log('%s: Transaction completed: %s', trx.txid, sql);
  throw new Error(
